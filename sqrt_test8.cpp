...
{
    //cout << "I am printing " << num.size() << endl;
    if(num.size() == 0)
    {
        return num;
    }
...
void change_base(pair<vector<int>, int> arr, int start_pres, int base)
{
    ...
    {
        x = multiply(base_v.first, n.first, base_v.second, n.second, 10, 100);

        n = addition(x.first, {arr.first[i]}, x.second, 0, 10, 100);
...
void pi(int total_precision, int base)
{
    ...
    cout << endl;
    change_base(p_next, p_next.first.size() - p_next.second - 1, base);
    return;
    //cout << endl;
    //cout << "root point: " << y[1][0] << endl;
    //x.first.clear();
